...
class Ynote_Invoice{
    ...
    public function sendMailInvoice($mailClient,$invoice){
            ...
            $message=str_replace('{{linkInvoice}}',"https://www.broadband.cm/facture/paie-facture.php?id=".$invoice['codeInvoice'], $message);

            $ccEmail = array("sales@y-note.cm","contact@broadband.cm");
            $email = $invoice['emailClient'];
            if($mailClient==0){
               $email = "contact@broadband.cm";
    ...
    public function saveInvoiceAction($nom,$invoice,$email,$tel,$sendMail,$montant){
        ...
        $request = "INSERT INTO Invoice (nomClient,telClient,invoiceNumber,emailClient,sendMail,montant,codeInvoice) VALUES ('".$nom."', '".$telClient."','".$invoice."', '".$email."','".$sendMail."','".$montant."','".md5(uniqid(rand(), true))."');";
